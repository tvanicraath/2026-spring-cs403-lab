...
class LitTab:
    ...
    def assign_addresses(self, current_locctr: int) -> int:
        ...
        for literal, entry in self.literals.items():
            # In Python 3.7+ dicts maintain insertion order
            if entry.address is None:
                entry.address = current_locctr
                current_locctr += entry.length
        return current_locctr
        
    def get_address(self, literal: str) -> int:
        # Called during Pass 2: Returns the assigned address.
        if literal in self.literals:
            addrs = self.literals[literal].address
            if addrs is not None:
                return addrs
            else:
                raise ValueError(f"Address for literal {literal} not assigned yet.")
        else:
            raise KeyError(f"Literal {literal} not found in Literal Table.")